...
fn lola_run() {
    ...
    let producer_builder =
        runtime.producer_builder::<com_api_gen_lola::VehicleInterface>(InstanceSpecifier {
    ...
    let sample = uninit_sample.write(com_api_gen_lola::Tire {});
    sample.send().unwrap();

    // Create service discovery
    let consumer_discovery =
        runtime.find_service::<com_api_gen_lola::VehicleInterface>(InstanceSpecifier {
            specifier: "My/Funk/ServiceName".to_string(),
        });
...
fn mock_run() {
    ...
    let producer_builder =
        runtime.producer_builder::<com_api_gen_mock::VehicleInterface>(InstanceSpecifier {
            specifier: "My/Funk/ServiceName".to_string(),
        });
    let producer = producer_builder.build().unwrap();
    let offered_producer = producer.offer().unwrap();
    ...
    // Create service discovery
    let consumer_discovery =
        runtime.find_service::<com_api_gen_mock::VehicleInterface>(InstanceSpecifier {
            specifier: "My/Funk/ServiceName".to_string(),
        });
    let available_service_instances = consumer_discovery.get_available_instances().unwrap();
